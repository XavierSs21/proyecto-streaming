...
const SessionForm = ({ mode, onSubmit }) => {
  const form = useForm({
    defaultValues:
      mode === "login"
        ? { correo: "", password: "" }
    ...
    onSubmit: async ({ value }) => {
      await onSubmit(value); // ← AQUÍ SÍ SE EJECUTA EL SUBMIT ✔️
    },
  });
  ...
  return (
    <form
      onSubmit={(e) => {
        e.preventDefault();
        form.handleSubmit(); // ← Submit REAL ✔️
      }}
      className="space-y-5"
    >
      {mode === "register" && (
      ...
      <Button className="w-full bg-amber-500 hover:bg-amber-600 text-black font-semibold h-11">
        {mode === "login" ? "Entrar" : "Crear Cuenta"}
      </Button>
    </form>
  );
