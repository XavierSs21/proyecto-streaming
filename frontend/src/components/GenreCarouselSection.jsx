...
export default function GenreCarouselSection({ genre, title }) {
  ...
  return (
    <section className="mt-12">
      ...
      <Carousel opts={{ align: "start", loop: movies.length > 4 }} className="w-full relative">
        <CarouselContent>
          {movies.map((movie) => (
            <CarouselItem
              key={movie._id}
            ...
            >
              <Card
                role="button"
                tabIndex={0}
                onClick={() => navigate(`/movie/${movie._id}`)} // ✅ AQUI
                onKeyDown={(e) => {
                  if (e.key === "Enter" || e.key === " ") navigate(`/movie/${movie._id}`);
                }}
                className="cursor-pointer overflow-hidden border-0 bg-transparent p-0 transition-transform duration-300 ease-in-out hover:scale-105 hover:shadow-xl"
              >
                <CardContent className="p-0">
                  <img
                    src={movie.thumbnailUrl || fallbackThumb}
